...
{
  constructor(border, EOL, ansi)
  {
    this.bottomLeft           = ansi(border.bottomLeft)
    this.teeLeft              = ansi(border.teeLeft)
    this.lastBranch           = ansi(border.bottomLeft + border.horizontal + ' ')
    this.lastBranchWithChild  = ansi(border.bottomLeft + border.horizontal + border.horizontal + border.teeUp + border.horizontal + ' ')
    this.midBranch            = ansi(border.teeLeft    + border.horizontal + ' ')
    this.midBranchWithChild   = ansi(border.teeLeft    + border.horizontal + border.horizontal + border.teeUp + border.horizontal + ' ')
    this.deepIndent           = ansi(border.horizontal + border.horizontal + border.horizontal + border.horizontal + ' ')
    this.indentPipe           = ansi(border.vertical + '  ')
    this.indentPad            = '   '
    this.EOL                  = EOL
  }
  ...
  padLenForPairs(pairs)
  {
    ...
    {
      if(p.label.length > max) max = p.label.length
      const vk = this.kind(p.v)
      if(vk !== 'leaf' && !this.isEmptyContainer(p.v, vk)) allInline = false
    }

    return allInline ? max : 0
  }
